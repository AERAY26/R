...
# Chapter 9, Modern Statistical Methods for Astronomy with R Applications
# Feigelson & Babu, Cambridge Univ Press, 2012
# Penn State Summer School June 2019

setwd('/Users/amyray/Desktop/PennStateAstrostat')
...
par(mfrow=c(1,2))
plot(COMBO_loz, pch=20, cex=0.2, xlim=c(-22,-7), ylim=c(-2,2.5), 
...
image(COMBO_loz_sm, col=grey(13:0/15), xlab=expression(M[B]~~(mag)), 
   ...
   xaxp=c(-20,-10,2))
text(-18.5, -1, "Blue cloud", col='darkblue', pos=4, cex=0.8)
...
# Ok so I got a warning here:
# Warning message:
# 'plclust' is deprecated.
# Use 'plot' instead.
# See help("Deprecated") also the name Valdimir Vapnik is important for some reason
...
# in Appendix C of Feigelson & Babu (2012)
...
###
### START SEQUENCE OF SIX CLASSIFIERS
### Plot predicted classes of test set and cross-validation on 20% of training set
###

# Linear discriminant analysis

library(MASS)
SDSS_lda <- lda(SDSS_train80[,1:4], as.factor(SDSS_train80[,5]))
SDSS_train20_lda <- predict(SDSS_lda, SDSS_train20[,1:4])
SDSS_test_lda <- predict(SDSS_lda, SDSS_test[,1:4])

par(mfrow=c(2,1))
plot(SDSS_test[,1],SDSS_test[,2], xlim=c(-0.7,3), ylim=c(-0.7,1.8), pch=20, 
   col=SDSS_test_lda$class, cex=0.5, main='', xlab='u-g (mag)', ylab='g-r (mag)') 
plot(jitter(as.numeric(SDSS_train20_lda$class), factor=0.5), jitter(as.numeric
   (SDSS_train20[,5]), factor=0.5), pch=20, cex=0.5, xlab='LDA class',
...
SDSS_test_knn4 <- knn(SDSS_train80[,1:4], SDSS_test, as.factor(SDSS_train80[,5]), k=4, prob=T)
SDSS_train20_knn4 <- knn(SDSS_train80[,1:4], SDSS_train20[,1:4], as.factor(SDSS_train80[,5]), k=4, prob=T)

plot(SDSS_test[,1], SDSS_test[,2], xlim=c(-0.7,3), ylim=c(-0.7,1.8), pch=20, 
   col=SDSS_test_knn4, cex=0.5, main='', xlab='u-g (mag)', ylab='g-r (mag)') 
plot(jitter(as.numeric(SDSS_train20_knn4), factor=0.5), jitter(as.numeric
   (SDSS_train20[,5]), factor=0.5), pch=20, cex=0.5, xlab='k-nn class',
   ylab='True class', xaxp=c(1,3,2),yaxp=c(1,3,2))
...
SDSS_nnet <- neuralnet(Class ~ u_g + g_r + r_i + i_z, SDSS_train80, hidden=2, rep=5)
plot(unlist(SDSS_nnet$net.result[5]), pch=20, cex=0.2)
# Improved performance with hidden=8, rep=10, 35 CPU-minutes computation


# Classification And Regression Tree

library('rpart')
SDSS_rpart_mod <- rpart(SDSS_train80[,5] ~., data=SDSS_train80[,1:4])
summary(SDSS_rpart_mod)
str(SDSS_rpart_mod)
SDSS_rpart_test_pred <- predict(SDSS_rpart_mod, SDSS_test)
SDSS_rpart_train_pred <- predict(SDSS_rpart_mod, SDSS_train20)

plot(SDSS_test[,1], SDSS_test[,2], xlim=c(-0.7,3), ylim=c(-0.7,1.8), pch=20, 
   col=round(SDSS_rpart_test_pred), cex=0.5,
   main='', xlab='u-g (mag)', ylab='g-r (mag)')
plot(jitter(SDSS_rpart_train_pred, factor=5), jitter(SDSS_train20[,5]), pch=20, 
...
library(MASS)
SDSS_RF <- randomForest(as.factor(Class) ~ u_g + g_r + r_i + i_z, data=SDSS_train80, mtry=2, importance=TRUE, do.trace=100)
print(SDSS_RF)
SDSS_RF_test_pred <- predict(SDSS_RF, SDSS_test)
SDSS_RF_train_pred <- predict(SDSS_RF, SDSS_train20)

plot(SDSS_test[,1], SDSS_test[,2], xlim=c(-0.7,3), ylim=c(-0.7,1.8), pch=20, 
   col=SDSS_RF_test_pred, cex=0.5,
   main='', xlab='u-g (mag)', ylab='g-r (mag)')
plot(jitter(as.numeric(SDSS_RF_train_pred), factor=1), jitter(SDSS_train20[,5]), pch=20, 
   cex=0.5, xlab='Random Forest class', ylab='True class',yaxp=c(1,3,2))


# Support Vector Machine model, prediction and validation

install.packages('e1071') ; library(e1071)
SDSS_svm_mod <- svm(SDSS_train80[,5] ~.,data=SDSS_train80[,1:4],cost=100, gamma=1)
summary(SDSS_svm_mod) ; str(SDSS_svm_mod) 
SDSS_svm_test_pred <- predict(SDSS_svm_mod, SDSS_test)
SDSS_svm_train_pred <- predict(SDSS_svm_mod, SDSS_train20)

plot(SDSS_test[,1], SDSS_test[,2], xlim=c(-0.7,3), ylim=c(-0.7,1.8), pch=20, 
   col=round(SDSS_svm_test_pred), cex=0.5, main='',
   xlab='u-g (mag)', ylab='g-r (mag)')
plot(SDSS_svm_train_pred, jitter(SDSS_train20[,5]), pch=20, cex=0.5, 
   xlab='Support Vector Machine class', ylab='True class', yaxp=c(1,3,2))

###
### Save SVM classification of the test set for scientific study
###
